...
+ 174K：2串3.7V三元锂输入，6V LED，极亮14.5A（适用于DFEx-SuperLED+ QV7007I Gen1）
+ 165K：3串3.7V三元锂输入，6V LED，极亮18A（适用于DFEx-SuperLED+ FI1048D Gen1）
+ 154K：2串3.7V三元锂输入，6V LED，极亮18A（适用于DFEx-SuperLED+ FI1048D Gen1）
+ 147K：3串3.7V三元锂输入，6V LED，极亮10A（适用于SFQ75.3-6V or CREE XHP70.2 or CREE XHP70.3HI/HD）
+ 133K：2串3.7V三元锂输入，6V LED，极亮10A（适用于SFQ75.3-6V or CREE XHP70.2 or CREE XHP70.3HI/HD）
+ 124K：3串3.7V三元锂输入，3V LED，极亮20A（适用于Luminus SBT90.2 or NBT160.2 or SFQ75.3）
+ 113K：2串3.7V三元锂输入，3V LED，极亮20A（适用于Luminus SBT90.2 or NBT160.2 or SFQ75.3）
...
**需要特别注意的是，配置电阻不能开路和短路，阻值需要在10K至200K之间。如果该电阻阻值不合适或者空焊，驱动将会拒绝启动并报错。为了避免驱动的低电压保护失效引起电池损坏或者错误的提示LED发生故障，请严格按照实际的硬件情况选取该配置电阻的阻值。**
...
+ 闪烁10次(Fault_MPUHang) 驱动检测到MCU故障挂死并触发强制复位。如果该故障第一次出现，您可重新刷入固件。仍然无法解决则需要更换MCU。
